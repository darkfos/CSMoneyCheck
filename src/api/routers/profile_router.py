...
async def profile_items_data(
    ...
    logger.info(msg="Получение данных о инвентаре пользователя", extra=user_config)
    redis_data = await redis.get_value(key="profile_data_{}".format(user_data.get("sub")))
    ...
    if not req:
        raise HTTPException(
            ...
            detail="No found user profile"
        )

    await redis.set_key(key="profile_data_{}".format(user_data.get("sub")), value=json.dumps(req)) # noqa
    return req
...
async def item_data(
    ...
    if not req:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No found item"
        )

    await redis.set_key(key="item_data_{}".format(item_name), value=json.dumps(req)) # noqa
    return req
