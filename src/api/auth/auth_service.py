...
class AuthService:
    ...
    async def create_token(
        cls, data: dict, type_token: Literal["access", "refresh"]
    ) -> str:
        match type_token:
            case "access":
                data.update(
                    {
                        "exp": (
                            ...
                            + timedelta(minutes=AuthSettings.JWT_SECRET_LIVE)
                        )
                    }
                )
                ...
                data.update(
                    {
                        "exp": (
                            datetime.now(timezone.utc)
                            + timedelta(days=AuthSettings.JWT_REFRESH_LIVE)
                        )
                    }
                )
    ...
    async def decode_token(
        cls, token: str, type_token: Literal["access", "refresh"]
    ) -> Union[dict, bool]:
        match type_token:
            case "access":
                try:
    ...
    async def verify_user(cls, req: Request, res: Response):
        ...
        if "access_token" in cookies and "refresh_token" in cookies:
            decode_access_token = await cls.decode_token(
                token=cookies.get("access_token"), type_token="access"
            )
