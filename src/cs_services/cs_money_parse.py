...
class CSMoneyService:
    ...
    def save_data(self, item_name: str) -> bool:
        ...
        if req.status_code == 200:
            with open("src/cs_services/js_data_items/csmoney.json", "w") as file:
                json.dump(req.json(), file, indent=4)
                return True

        return False

    def get_all_data_by_itemname(self, item_name: str) -> Union[dict[str, Union[str, int]], bool]:
        
        is_saved: bool = self.save_data(item_name=item_name)

        if is_saved:
            
            with open("src/cs_services/js_data_items/csmoney.json", "r") as file:
